 ...
 * service ids. Multi-process services label pods `<service>-<process>` (af-api-web/worker/admin);
 * keying by that raw `app` label left them absent from the map and falling back to the fleet baseline,
 * which intermittently SKIPPED their deploy — the deploy-gap that left admin-only af-api changes
 */
describe("serviceFromPodLabels", () => {
  it("strips the process suffix for multi-process services", () => {
    expect(serviceFromPodLabels("af-api-web", "web")).toBe("af-api");
    expect(serviceFromPodLabels("af-api-worker", "worker")).toBe("af-api");
    expect(serviceFromPodLabels("af-api-admin", "admin")).toBe("af-api");
  });
  ...
  it("does not strip when the app label doesn't end with the process suffix", () => {
    ...
    expect(serviceFromPodLabels("af-api", "web")).toBe("af-api");
